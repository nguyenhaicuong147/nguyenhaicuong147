...
module.exports.run = async function({ api, event, Users, Threads, Currencies }) {
  ...
  if (money1 < 1000 || isNaN(money1)) return api.sendMessage(`Bạn phải có ít nhất 1000$ để có thể trộm!`, threadID, messageID);
  var mention = Object.keys(event.mentions)[0];
  if (!mention) return api.sendMessage(`Vui lòng tag mục tiêu!`, threadID, messageID);
  const botID = api.getCurrentUserID();
  if (mention == botID) return api.sendMessage("Bạn đéo được phép trộm bot -.-", threadID, messageID);
